...
This automated email was generated by http://umd-repo.jf.intel.com/git/users/$reposuser/scripts/kernelcron.sh
The build RPMS spec was based on http://umd-repo.jf.intel.com/git/users/$reposuser/scripts/kernel-umd-source.spec.profile
!)
...
# let remote host do git stuff       
       ...
       git-clone -n -l /home/repos/OpenSuSE10.2/package-meta-data.git package-meta-data       
       cd package-meta-data
       git-checkout UMD
!)
   echo "$commands" > commands.sh
   scp commands.sh $reposhost:$myrepository/
   ssh $reposhost "cd $myrepository; sh commands.sh; rm commands.sh"
...
#  split version string for spec.profile
   ...
   if [ $? -eq 0 ]; then
      ...
      scp $reposhost:$myrepository/package-meta-data/Development/Sources/kernel-umd-source/files/* /usr/src/packages/SOURCES/
      cp kernel-umd-source.spec /usr/src/packages/SPECS/
      cp $newver $patch /usr/src/packages/SOURCES/
      cd /usr/src/packages
      rm -rf /usr/src/packages/RPMS/*
      rm -rf /usr/src/packages/SRPMS/*
      rm /usr/src/packages/*.log
      ...
      scp /usr/src/packages/$logfile $reposhost:$myrepository/logs/
...
# report good news to team
         ...
         scp -r /usr/src/packages/RPMS $reposhost:$myrepository/
         scp -r /usr/src/packages/SRPMS $reposhost:$myrepository/
         ...
         EmailNotification "successful" "$url$newver" "$check_url3$patch" $(echo $patches | sed -e "s/Patch.\{1,5\}: //g") \
                           "http://umd-repo.jf.intel.com/git/users/$reposuser/logs/$logfile" \
                           "http://umd-repo.jf.intel.com/git/users/$reposuser/" \
                           "http://umd-repo.jf.intel.com/git/users/$reposuser/package-meta-data.git/" \
                           "http://umd-repo.jf.intel.com/repos/?p=users/xdu1/package-meta-data.git;a=shortlog;h=UMD"
