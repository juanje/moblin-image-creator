...
if [ $testmode -eq 1 ]; then
   ...
   echo "run mode"
   mailto="rob.rhoads@intel.com, feng.tang@intel.com, frank.li@intel.com, alek.du@intel.com"
   #mailto="alek.du@intel.com"
fi
...
The build RPMS spec was based on http://umd-repo.jf.intel.com/git/users/$reposuser/$target/scripts/kernel-mid-source.spec.profile
!)
...
# save old patches info
   scp $reposhost:$myrepository/package-meta-data/Development/Sources/kernel-mid-source/specs/kernel-mid-source.spec kernel-mid-source.spec
   echo "getting release info from spec file..."
   pkgrelease=$(cat kernel-mid-source.spec | sed -n -e "/^Release:/p")
   echo "getting patches info from spec file..."
   patches=$(cat kernel-mid-source.spec | sed -n -e "/Patch0/d" -e "/^Patch/p")
   patchesapply=$(cat kernel-mid-source.spec | sed -n -e "/^%patch0/d" -e "/^%patch/p")
...
#  split version string for spec.profile
   ...
   (sed -e "s/@TARGET@/$target/g" \
        ...
        $homedir/$progdir/kernel-mid-source.spec.profile
   )> kernel-mid-source.spec
   ...
   if [ $? -eq 0 ]; then
      ...
      scp $reposhost:$myrepository/package-meta-data/Development/Sources/kernel-mid-source/files/* /usr/src/$rpmdir/SOURCES/
      cp kernel-mid-source.spec /usr/src/$rpmdir/SPECS/
      ...
      if [ $testmode -eq 0 ]; then
         rpmbuild -ba ./SPECS/kernel-mid-source.spec --target=i586 > $logfile 2>&1
      else
...
# copy to remote package-meta-data for commit
      cd $homedir/$progdir/
      scp kernel-mid-source.spec.profile $reposhost:$myrepository/scripts/
      scp kernelcron.sh $reposhost:$myrepository/scripts/
      scp kernel-mid-source.spec $reposhost:$myrepository/package-meta-data/Development/Sources/kernel-mid-source/specs/
      ssh $reposhost "cd $myrepository/package-meta-data;git-update-index Development/Sources/kernel-mid-source/specs/kernel-mid-source.spec"
      if [ -n "$patch" ]; then
         scp $patch $reposhost:$myrepository/package-meta-data/Development/Sources/kernel-mid-source/files/
         ssh $reposhost "cd $myrepository/package-meta-data;git-update-index --add Development/Sources/kernel-mid-source/files/$patch"
      fi
